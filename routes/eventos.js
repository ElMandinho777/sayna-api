...
router.put('/:id/completar', async (req, res) => {
  try {
    ...
    await pool.query("UPDATE Eventos SET estado = 'completado' WHERE id_evento = ?", [id]);
    // Libera mobiliario asignado (si lo deseas)
    await pool.query(
      "UPDATE Mobiliario SET estado = 'disponible' WHERE id_mobiliario IN (SELECT id_mobiliario FROM Evento_Mobiliario WHERE id_evento = ?)",
      [id]
    );
